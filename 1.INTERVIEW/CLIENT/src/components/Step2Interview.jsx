...
// TODO: adjust this import to wherever your app defines the API base URL
import { serverUrl } from "../App.jsx";

function Step2Interview({ interviewData, onFinish }) {
  // Pull the real data out of the prop instead of relying on undefined globals
  ...
  useEffect(() => {
    ...
    // then again whenever the browser finishes loading its voice list.
    loadVoices();
  ...
  const finishInterview = async () => {
    ...
    try {
      const result = await axios.post(
        serverUrl + "/api/interview/finish", // TODO: set your real "finish" endpoint
        { interviewId },
        { withCredentials: true }
      );
    ...
    <div className="min-h-screen bg-linear-to-br from-emerald-50 via-white to-teal-100 flex items-center justify-center p-4 sm:p-6">
      <div className="w-full max-w-350 min-h-[80vh] bg-white rounded-3xl shadow-2xl border border-gray-200 flex flex-col lg:flex-row overflow-hidden">
        ...
        <div className="w-full lg:w-[35%] bg-white flex flex-col items-center p-6 space-y-6 border-r border-gray-200">
          <div className="w-full overflow-hidden max-w-md rounded-2xl shadow-xl">
            <video
              src={videoSource}
              key={videoSource}
              ref={videoRef}
          ...
          <div className="w-full max-w-md bg-white border border-gray-200 rounded-2xl shadow-md p-6 space-y-5">
            ...
            <div className="grid grid-cols-2 gap-6 text-center">
              <div>
                <span className="text-lg font-bold text-emerald-800"> {currentIndex + 1}</span>
                <span className="text-xs text-gray-500">Current Question</span>
              </div>

              <div>
                <span className="text-lg font-bold text-emerald-800"> {questions.length}</span>
        ...
        <div className="flex-1 flex flex-col p-4 sm:p-6 md:p-8 relative">
          ...
          {!feedback ? (
            ...
            <motion.div
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
              className="mt-6 bg-emerald-50 border border-emerald-200 rounded-2xl shadow-sm p-5"
            >
