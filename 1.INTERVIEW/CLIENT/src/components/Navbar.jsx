...
function Navbar() {
  ...
  return (
    <div className="bg-[#f3f3f3] flex justify-center px-4 pt-6">
      <motion.div
        ...
        className="w-full max-w-6xl bg-white rounded-[24px] shadow-sm border border-gray-200 px-8 py-4 flex items-center justify-between relative"
      >
        <div className="flex items-center gap-6 relative">
          <div className="bg-black text-white p-2 rounded-lg">
            <BsRobot size={18} />
          </div>
          <h1 className="hidden md:block text-lg font-semibold">IntelliHireIQ.AI</h1>
        </div>
         <div className="flex items-center gap-6 relative">
  <div className="relative">
    <button
      onClick={() => { if(!userData)
         {setShowAuth(true)
           return;} setShowCreditPopup(!showCreditPopup); setShowUserPopup(false);}}
    ...
    >
      Buy More Credits
          ...
          setShowUserPopup(!showUserPopup);
            ...
            {showUserPopup && (<div className='absolute right-0 mt-3 w-48 bg-white shadow-x1 border border-grey-200 rounded-x1 p4 z-50'>
              <p className='text-md text-blue-500 font-medium mb-1'>{userData?.name}</p>
            <button className ='w-full text-left text-sm py-2 hover:text-block text-gray-600'>
              Interview History
            </button>
